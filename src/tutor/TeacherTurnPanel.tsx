...
export function TeacherTurnPanel({ state, onRetry, onHint, onAdvance, onSaveRetry }: TeacherTurnPanelProps) {
  ...
  if (turn.memory_write_failed) {
    return <section className="mn-teacher-turn mn-teacher-turn--save" data-testid="teacher-memory-recovery" role="alert"><h2 ref={headingRef} tabIndex={-1}>答案已收到，進度還沒存好</h2><p>先把這次學習安全存好，就能繼續。</p><div className="mn-teacher-turn__actions"><button type="button" className="mn-button mn-button--primary" data-testid="teacher-save-retry" onClick={onSaveRetry}>再存一次</button></div></section>;
  }
  return (
    <section className="mn-teacher-turn" data-testid="teacher-turn-result" data-verdict={turn.verdict} aria-labelledby="teacher-turn-title">
      ...
      {action === "next" && turn.selection_reason && <p className="mn-teacher-turn__next-reason" data-testid="teacher-next-reason">老師接著安排：{turn.selection_reason}</p>}
