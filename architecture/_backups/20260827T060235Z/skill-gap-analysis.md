...
1. **Curriculum source authority.** Who curates the Taiwan K–12 curriculum YAML that curriculum-agent will read? MentorNest internal, contracted educator, or external dataset (with licensing review)?
2. **Question licensing posture.** Are we allowed to ingest any portion of publisher textbooks into the question bank, or only teacher-authored / openly-licensed questions? This drives whether the question bank is mostly empty for the first year.
3. **Student identity beyond nickname.** Current profiles have `display_name` and `grade` only. Do we need school name, teacher, exact age? AGENTS.md currently forbids asking these unless explicitly approved — confirm this is the policy.
4. **adaptivetest verdict.** The third-party `adaptivetest-skill` is installed but unvetted. Either (a) remove it from disk, or (b) run it through `skill-vetter` and decide whether to formally register and isolate it behind a child-data firewall. Today it sits in an ambiguous state.
5. **Promotion of `student-profile` skill and `adaptivetest` into `skills-registry.yaml`.** They are on disk but not in the registry — pick a canonical identifier for each and add a registry entry (or delete).
