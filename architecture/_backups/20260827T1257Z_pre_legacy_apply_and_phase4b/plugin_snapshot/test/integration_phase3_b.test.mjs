...
// ─────────────────────────────────────────────────────────────────────────────
// Per-test cleanup — guarantees no `student_t_b` artifacts persist across runs.
// Production ledgers (`student_001.jsonl`, `student_002.jsonl`) are NEVER touched.
// ─────────────────────────────────────────────────────────────────────────────
test.after(async () => {
  await fs.unlink(EVIDENCE_TEST_B).catch(() => {});
  await fs.rm(path.join(WORKSPACE, "data/mastery", `${TEST_STUDENT_B}.json`), { recursive: true, force: true }).catch(() => {});
  await fs.rm(path.join(WORKSPACE, "data/curriculum-progress", `${TEST_STUDENT_B}.jsonl`), { recursive: true, force: true }).catch(() => {});
  await fs.rm(path.join(WORKSPACE, "data/mastery-backfill", TEST_STUDENT_B), { recursive: true, force: true }).catch(() => {});
});
