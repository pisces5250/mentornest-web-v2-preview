...
export const COPY_ZH_TW = Object.freeze({
  ...
  welcome: {
    ...
    privacy_note: "資料只用於孩子個人化學習，不自動分享給其他學生或外部服務。",
  },
  ...
  // school_progress is OWNED by curriculum-agent and surfaced to the parent
  // for confirmation — never authored by the parent directly.
  // The wording below (per 2026-08-27 product decision) acknowledges the
  // parent's input flows through curriculum-agent, not as a raw payload field.
  school_progress: {
    title: "學校老師目前大約教到哪個單元",
    description: "學校老師目前大約教到哪個單元；不確定可跳過，之後再更新。",
    hint: "例如：南一版第幾單元",
    required: false,
    skip_label: "不確定，之後再更新",
    owned_by: "curriculum-agent",
    parent_role: "approver",
    flow: "parent-rough-estimate → curriculum-agent-mapping → confirmed_progress",
  },
  learning_goals: {
  ...
  // Global privacy copy — exposed via parent_setup_schema_copy.
  // Per 2026-08-27 product decision: every parent-facing surface carries
  // this verbatim. UI is responsible for rendering it next to any
  // school_name / class_name advanced-only field.
  privacy: Object.freeze({
    title: "隱私承諾",
    lines: [
      "資料只用於孩子個人化學習，不自動分享給其他學生或外部服務。",
      "MentorNest 不會主動詢問學校、班級，除非家長自行輸入。",
      "學習聲音只留下文字重點，不會保留原始音檔。",
    ],
  }),
});
...
/**
 * Get the locale-specific copy object (only zh-TW supported in V1).
 *
 * @param {{ locale?: string }} opts
 * @returns {{ ok: true, locale: string, copy: object, invariants: object } | { ok: false, reason: string }}
 */
export function getParentSetupCopy({ locale = "zh-TW" } = {}) {
  if (locale !== "zh-TW") {
    return { ok: false, reason: `locale ${locale} not supported in V1 (only zh-TW)` };
  }
  return {
    ok: true,
    locale,
    ...
    invariants: {
      ...
      learning_memory_audio: "forbidden",
      privacy_copy_present: true,
      privacy_lines: COPY_ZH_TW.privacy.lines,
    },
  };
}
