...
test.after(async () => {
  // Comprehensive cleanup: remove per-test-student evidence/mastery/curriculum/backfill artifacts
  // so subsequent runs start from a clean slate. Production ledgers (student_001/002) untouched.
  const testStudents = [
    "student_test_a",
    "student_test_b",
    "student_test_e",
    "student_phase3_e_isolated",
    "student_phase3_e_a",
    "student_phase3_e_b",
  ];
  for (const s of testStudents) {
    await fs.unlink(path.join(WORKSPACE, "data/mastery-evidence", `${s}.jsonl`)).catch(() => {});
    await fs.unlink(path.join(WORKSPACE, "data/mastery", `${s}.json`)).catch(() => {});
    await fs.rm(path.join(WORKSPACE, "data/curriculum-progress", `${s}.jsonl`), { recursive: true, force: true }).catch(() => {});
    await fs.rm(path.join(WORKSPACE, "data/mastery-backfill", s), { recursive: true, force: true }).catch(() => {});
  }
});
...
test("plugin loads with all 13 Phase 3-E social_studies tools (total = 105)", async () => {
  ...
  assert.equal(tools.length, 132, `expected 132 tools (130 prior + 2 Phase 4A), got ${tools.length}`);
});
