...
## 11. Skill Gap Analysis — Phase 4B KP classifier + License + Local PDF (2026-08-27T1518Z)

### 11.1 What Phase 4B shipped (skills-registry additions)

| Skill | Status | Tool | Owner agent |
|---|---|---|---|
| Knowledge Point classifier V1 | SHIPPED 20260827T1518Z | `kp_classify` | question-bank-curator |
| Source license extractor | SHIPPED 20260827T1518Z | `license_extract` | question-bank-curator |
| License reviewer (Verified Bank gate) | SHIPPED 20260827T1518Z | `license_review` | question-bank-curator + question-quality-agent |
| Raw → Verified pipeline orchestrator | SHIPPED 20260827T1518Z | `raw_question_ingest_full_pipeline` | question-bank-curator |
| Local selectable-text PDF adapter | SHIPPED (extension of Phase 4A) | `raw_question_ingest` (pdf path) | question-bank-curator |

### 11.2 Decisions still owed (after Phase 4B)

| ID | Decision | Trigger timing |
|---|---|---|
| R9 | PDF text-extraction library choice: stay with crude `ascii_fallback`, OR add `pdf-parse` (CJS) / `pdfjs-dist` (full-text) when first real scanned-text PDF arrives in production | Phase 4C kickoff (only when actual scanned-text PDF is observed) |
| R10 | License provenance UI: how a parent / child sees the license decision for a question they're shown | Phase 5 design phase |
| R11 | Whether `license_review.allow_private_only` questions stay in the student's personal private pool forever or get re-reviewed at TTL expiry | Phase 4C kickoff |

### 11.3 Hard rules (carry-forward to Phase 4C)

- No second Verified Bank writer. `question_quality_agent_verify` is the only writer.
- No cloud OCR. Scanned / image-only PDFs remain `unsupported_in_round_4a`.
- No `Tesseract` primary. No `pdf-parse` if not already in `node_modules`.
- No Tesseract OCR skill install (`chinese-handwriting-ocr`, `aidenwu0209-paddleocr-skills`, `beagle-docling-wrapper`, `pix2tex-production` remain forbidden).
- `UNKNOWN` and `COMMERCIAL_RESTRICTED` cannot enter the shared Verified Bank.
- KP classifier: confidence `< 0.6` → REJECT from Verified Bank (raw pool keeps with `status: low_confidence|unknown`).

### 11.4 Phase 5 install gate (UNCHANGED, repeated for clarity)

All 6 conditions in § 10.8 must still be green before any Skill install. Phase 4B did NOT install any new Skill — only added tools backed by local libraries already in `node_modules` or pure JS.

---

## § 12 — Skill Gap Analysis — Phase 4C

### Phase 4C shipped (2026-08-27T1615Z)
| Tool | Lib module | Status |
|---|---|---|
| `question_coverage_analyzer` | lib/question_coverage_analyzer.mjs | shipped |
| `question_generation_planner` | lib/question_generation_planner.mjs | shipped |
| `difficulty_controller` | lib/difficulty_controller.mjs | shipped |
| `variation_generator` | lib/variation_generator.mjs | shipped |
| `duplicate_detector` | lib/duplicate_detector.mjs | shipped |
| `answer_verifier` | lib/answer_verifier.mjs | shipped |
| `ai_authoring_orchestrator_run` (extended in place) | lib/ai_question_authoring_orchestrator.mjs | shipped |

### Still owed (carried forward from R1-R11)
- R1 Math input library (Phase 5 kickoff)
- R2 Drag/drop library (Phase 5 kickoff)
- R3 SVG layer integration (Subject Specialist handoff)
- R4 Phase 6 handwriting recognition tech
- R5 G1-G2 default TTS voice_id
- R6 MentorNest icon style guide
- R7 high_contrast palette token values
- R8 SaaS usability upgrade timing
- R9 PDF parser (Phase 4C kickoff — still deferred)
- R10 License provenance UI (Phase 5)
- R11 PRIVATE lifecycle (Phase 4C kickoff — still deferred)
