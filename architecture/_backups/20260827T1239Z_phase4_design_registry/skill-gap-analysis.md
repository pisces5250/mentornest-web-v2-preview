...
### 10.6 Decisions the human must make before installation

1. **Install Storybook for design system documentation?** Storybook is the de-facto standard for documenting a Design System, but it pulls in React/Vue/Angular peer deps. Decision: Phase 5 web-v2 stack picks the framework first; Storybook install deferred.
2. **Install axe-core / Pa11y for accessibility regression?** Either is technically a single-line `npm install`. Decision deferred until Phase 5 picks the test harness.
3. **Local handwriting recognition (Phase 6): sherpa-onnx vs Tesseract.js vs ?** Sherpa-onnx already powers STT; extending it to handwriting is consistent but NOT trivial. Tesseract.js is large (~30MB) and historically weaker on handwriting. Decision deferred to Phase 6.
4. **Color-blind palette generation tool** — 3rd-party (Sim Daltonism, Stark) are macOS apps for design-time review only; not automated. Self-build a CI hook using chroma.js to validate contrast ratios in design-tokens.yaml.
5. **Color-blind simulator at runtime** — should the runtime pick the override palette based on user's browser/OS preference (`prefers-color-scheme`)? OR always serve the same palette and rely on system-level color filters? Decision owed before Phase 5 ships.
